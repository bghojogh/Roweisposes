...
if ~display_projected_LDA_mode
    [eigvector, eigvalue_vector] = eig(R1, R2, 'vector');
else
   % [eigvector, eigvalue_vector] = eig((R2 \ R1) + (10 * eye(size(R1,1))), 'vector');
    R1 = R1 ./ max(max(R1));
    R2 = R2 ./ max(max(R2));
    R1 = R1 + (1 * eye(size(R1,1)));
    R2 = R2 + (1 * eye(size(R2,1)));
    % [eigvector, eigvalue_vector] = eig((R2 \ R1) , 'vector');
    [eigvector, eigvalue_vector] = eig(R1, R2, 'vector'); 
end
